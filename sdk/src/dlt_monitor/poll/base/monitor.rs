...
enum WaitFor<'a, Status: BatchStatus> {
    ...
    Request(&'a dyn BatchStatusReader<Status = Status>, String, Vec<String>),
    ...
    Updater: BatchUpdater<Status = Status>,
{
    ...
    async fn make_batch_requests<'a>(&'a self) -> impl Iterator<Item = WaitFor<'a, Status>> {
...
pub trait PollingMonitorBuilder: Send {
    type Id: BatchId;
...
struct RunnablePollingMonitor<B: 'static + PollingMonitorBuilder> {
    builder: B,
}

impl<B: 'static + PollingMonitorBuilder> RunnablePollingMonitor<B> {
    pub fn new(builder: B) -> Self {
        RunnablePollingMonitor { builder }
    }

    pub fn run(self) -> BatchResult<RunningPollingMonitor> {
        ...
        let runtime_handle = std::thread::Builder::new()
            ...
            .spawn(move || {
                runtime.block_on(async move {
                    let monitor = self.builder.build();
...
mod mock {
    ...
    impl<T: Debug + Eq + Clone + Ord, R> CallTester<T, R> {
        ...
        pub fn call(&self, call: T) -> R {
            ...
            let index = protected_value
                ...
                .iter()
                .enumerate()
                .position(|(i, expected_call)| {
                    indices.contains(&i) && expected_call.is_some()
                })
    ...
    impl<T: Debug + Eq + Clone + Ord, R> CallAsserter<T, R> {
        ...
        pub fn assert(&mut self) {
            ...
            let mut expected = self.expected_calls
                    .iter()
                    .map(Option::from)
                    .collect::<Vec<_>>();
...
mod tests {
    ...
    use super::mock::{ TestBuilder, CallAsserter, CallTester };
    ...
    type ObserverCall = Event<FakeBatchId>;
    ...
    struct MockMonitorBuilder<'a> {
        construct: Box<dyn FnOnce() -> MockMonitor<'a> + Send>,
    }

    impl<'a> PollingMonitorBuilder for MockMonitorBuilder<'a> {
        type Id = FakeBatchId;
        type Status = FakeBatchStatus;
    ...
    impl<'b> Observer for MockObserver<'b> {
        type Event = Event<FakeBatchId>;

        fn notify<'a>(&'a self, _event: &'a Event<FakeBatchId>) -> BoxFuture<'a, ()> {
    ...
    async fn update_sync_correctly_updates_statuses() {
        ...
        let builder = MockMonitorBuilder {
        ...
        let runnable = RunnablePollingMonitor::new(builder);
